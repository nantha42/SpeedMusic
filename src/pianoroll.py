...
class Pianoroll:
    def __init__(self,measures):
        ...
        self.controls = {"h_zoom": 1, "timebar": 0, "bar_move_velocity": 0, "make_velocity_0": True,"nthnote":2}
    ...
    def update(self):
        ...
        #s = t+800
        for i in range(20, int(20*self.controls["h_zoom"]*self.measures*32)+30, int(20 * self.controls["h_zoom"])):
            ...
            if int(self.controls["timebar"]+i)>=20 and int(self.controls["timebar"]+i)<=800:
                #q+= str(" D")
                if fourth % 4 == 0:
                    py.gfxdraw.line(self.pianoroll, int(self.controls["timebar"]+i), 10, int(self.controls["timebar"]+i), self.height - 10, (255, 255, 255, 80))
                else:
                    py.gfxdraw.line(self.pianoroll, int(self.controls["timebar"]+i), 10, int(self.controls["timebar"]+i), self.height - 10, (255, 255, 255, 30))
            #print(q)
            fourth += 1
        ...
        index = 0
        for i in range(20, int(20*self.controls["h_zoom"]*(self.measures+1)*32), int(20 * self.controls["h_zoom"] * 32)):
    ...
    def draw_notes(self):
        for y,x in self.notes_index:
            ...
            if j+20*self.controls["h_zoom"]*(2**self.notes[y][x])-2 >=20 and j<800:
                py.draw.rect(self.pianoroll,(200,0,0),((x*20*self.controls["h_zoom"]+self.controls["timebar"])+20,y*10+10+2,20*self.controls["h_zoom"]*(2**self.notes[y][x])-2,10-2))
